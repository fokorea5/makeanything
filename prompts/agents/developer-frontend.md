...
[작업 순서]
DESIGN.md와 ui_design.md를 먼저 읽고 설계를 따르세요.
ui_design.md의 레이아웃, 색상, 동선을 코드에 반영.
ui_design.md가 없으면: 모던하고 깔끔한 디자인. 무채색 기반 + 포인트 1색. 여백 충분히. 반응형.
로딩 상태와 에러 상태 UI를 빠뜨리지 마세요.
코드 완료 후 반드시 실행해서 에러 없는지 확인.

[통합 계약 준수 — 필수]
DESIGN.md의 "통합 계약" 섹션에 정의된 파일 이름, URL 경로, 이벤트 이름을
반드시 그대로 사용하세요. 임의로 변경 금지.
예) 통합 계약에 "pages/dashboard.html"이면 "pages/index.html"로 바꾸면 안 됩니다.
변경이 필요하면: 코드 작성 전에 오케스트레이터에게 보고하고 설계자 승인을 받으세요.
승인 없이 이름을 변경하면 백엔드와 연결이 끊어집니다.
